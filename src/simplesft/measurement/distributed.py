"""Helpers for launching and aggregating DDP measurement runs."""
...
from .measure import _measure_local_peak_memory, aggregate_rank_results
...
def run_ddp_measurement(
    *, model: str | ModelSpec, config: TrainingConfig
    ...
    Args:
        model: Hugging Face model reference or model spec to measure.
        config: DDP training configuration to run.

    Returns:
        Aggregated max-per-rank measurement result.
    """

    with tempfile.TemporaryDirectory(prefix="simplesft-ddp-") as temp_dir:
        ...
        subprocess.run(
            torchrun_command(
                module_name="simplesft.distributed_measure",
...
def _setup_distributed_runtime() -> int:
    """Initialize the DDP worker runtime and return the local rank."""
...
def _gather_results(*, local_result: MemoryResult) -> list[dict[str, Any]]:
    """Gather serialized rank-local measurement results across all ranks."""
    ...
    return [cast_result for cast_result in gathered_results if cast_result is not None]


def main() -> None:
    """Run the torchrun worker entrypoint for DDP measurement."""

    parser = argparse.ArgumentParser(prog="python -m simplesft.distributed_measure")
    parser.add_argument("--input", required=True)
    parser.add_argument("--output", required=True)
    args = parser.parse_args()

    model, config = load_request(path=args.input)
    local_rank = _setup_distributed_runtime()
    local_result = _measure_local_peak_memory(
        model=model,
        config=config,
    ...
    if dist.get_rank() == 0:
        aggregated_result = aggregate_rank_results(
            ...
            aggregation_tag="ddp_aggregated",
            aggregation_assumption="DDP result aggregates the max across ranks.",
        )
