...
def _result(global_peak_bytes: int, distributed_mode: str, feasible: bool) -> MemoryResult:
...
def test_compare_measurement_to_estimate_reports_zero_error_for_equal_results() -> None:
    ...
    measured = _result(global_peak_bytes=10, distributed_mode="single_gpu", feasible=True)
    estimated = _result(global_peak_bytes=10, distributed_mode="single_gpu", feasible=True)
...
def test_compare_measurement_to_estimate_tracks_workspace_proxy_errors() -> None:
    ...
    assert comparison.workspace_proxy_relative_error["forward_workspace_proxy_bytes"] == (2 / 7)
...
def test_search_configurations_prefers_simpler_feasible_modes() -> None:
    ...
    result = search_configurations(
        ...
        configs=[
            TrainingConfig(tuning_mode="full_ft", distributed_mode="zero2"),
            TrainingConfig(tuning_mode="full_ft", distributed_mode="single_gpu"),
        ],
    )
